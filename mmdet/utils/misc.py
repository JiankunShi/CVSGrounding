...
def update_data_root(cfg, logger=None):
    ...
    Args:
        cfg (:obj:`Config`): The model configs need to modify
...
def get_test_pipeline_cfg(cfg: Union[str, ConfigDict]) -> ConfigDict:
    """Get the test dataset pipeline from entire configs.

    Args:
        cfg (str or :obj:`ConfigDict`): the entire configs. Can be a configs
            file or a ``ConfigDict``.

    Returns:
        :obj:`ConfigDict`: the configs of test dataset.
    """
