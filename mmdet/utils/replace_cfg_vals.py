...
def replace_cfg_vals(ori_cfg):
    ...
    Replace the "${key}" with the value of ori_cfg.key in the configs. And
    ...
    Args:
        ori_cfg (mmengine.config.Config):
            The origin configs with "${key}" generated from a file.

    Returns:
        updated_cfg [mmengine.configs.Config]:
            The configs with "${key}" replaced by the corresponding value.
    """
    ...
    # the type of ori_cfg._cfg_dict is mmengine.configs.ConfigDict
    updated_cfg = Config(
